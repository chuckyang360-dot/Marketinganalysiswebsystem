    ...
    <section className="w-full py-28" style={{ background: "#ffffff", borderTop: "1px solid #EAEAEA" }}>
      <div className="relative z-10 mx-auto px-6 lg:px-10" style={{ maxWidth: "1100px" }}>
        <div ref={ref} style={{ opacity: 0, transform: "translateY(24px)", transition: "opacity 0.7s ease, transform 0.7s ease" }}>
          ...
          <h2
            className="font-extrabold leading-[1.12] tracking-[-0.03em] mb-6"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {TRADITIONAL.map((step, i) => (
                  <div key={step} className="flex items-center gap-2">
                    <span className="px-3 py-1.5 rounded-lg text-[13px] font-medium" style={{ background: "#ffffff", color: "#888888", border: "1px solid #EAEAEA" }}>
                      {step}
                    </span>
                    {i < TRADITIONAL.length - 1 && <i className="ri-arrow-right-line text-[12px]" style={{ color: "#CCCCCC" }} />}
                  </div>
                ))}
              </div>
            ...
            <div className="rounded-2xl p-7 relative overflow-hidden" style={{ background: "rgba(123,97,255,0.05)", border: "1px solid rgba(123,97,255,0.2)" }}>
              <div className="flex items-center gap-2 mb-6">
                <span
                  className="px-2.5 py-1 rounded-md text-[11px] font-semibold uppercase tracking-wide"
              ...
              <div className="flex flex-wrap items-center gap-2">
                {OUR_PATH.map((step, i) => (
                  <div key={step} className="flex items-center gap-2">
                    ...
                    {i < OUR_PATH.length - 1 && <i className="ri-arrow-right-line text-[12px]" style={{ color: "rgba(123,97,255,0.4)" }} />}
                  </div>
                ))}
              </div>
              <p className="mt-5 text-[13px] font-medium" style={{ color: "#7B61FF" }}>3 步，直接到结果</p>
              <div
                className="absolute bottom-0 right-0 w-40 h-28 pointer-events-none"
