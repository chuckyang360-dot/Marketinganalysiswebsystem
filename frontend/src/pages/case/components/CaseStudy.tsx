...
import { AlertTriangle, ArrowDown, LineChart, Search, Wrench } from "lucide-react";
import type { LucideIcon } from "lucide-react";

export type CaseStepIconName =
  | "ri-alert-line"
  | "ri-search-eye-line"
  | "ri-tools-line"
  | "ri-line-chart-line";
    ...
    <div className="rounded-2xl overflow-hidden" style={{ background: "#ffffff", border: "1px solid #EAEAEA" }}>
      ...
      <div className="p-5 flex flex-col gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <ArrowDown size={12} color="#7B61FF" strokeWidth={2} />
          <div className="flex-1 h-px" style={{ background: "#EAEAEA" }} />
        </div>
        <div>
          <div className="flex items-center gap-2 mb-3">
          ...
          <div className={`flex flex-col lg:flex-row gap-8 items-start ${data.reversed ? "lg:flex-row-reverse" : ""}`}>
            <div className="flex-1 flex flex-col gap-4 min-w-0">
              {data.steps.map((step, i) => {
                ...
                return (
                  <div key={step.label} className="rounded-xl p-5 flex gap-4" style={{ background: style.bg, border: `1px solid ${style.border}` }}>
                    <div className="w-9 h-9 flex items-center justify-center rounded-lg shrink-0 mt-0.5" style={{ background: "#ffffff", border: `1px solid ${style.border}` }}>
                      {(() => {
                        const StepIcon = STEP_ICON_MAP[step.icon];
                        return StepIcon ? <StepIcon size={15} color={step.color} strokeWidth={2} /> : null;
                      })()}
                    </div>
